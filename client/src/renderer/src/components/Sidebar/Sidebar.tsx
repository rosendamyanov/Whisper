...
export const Sidebar = (): JSX.Element => {
  ...
  }, [fetchChats, user?.id, fetchPendingRequests])
      ...
      <div className="bg-white/[0.02] backdrop-blur-2xl border border-white/[0.05] p-5 rounded-3xl flex items-center justify-between shadow-xl">
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <div
              className={`absolute -bottom-1 -right-1 w-3.5 h-3.5 rounded-full border-[2.5px] border-[#0a0a0c] ${STATUS_COLORS[myStatus]}`}
            />
          </div>
          <div>
            <h2 className="font-semibold text-sm text-white">{user?.username || 'My Account'}</h2>
              ...
              return (
                ...
                >
                  <div className="relative flex-shrink-0">
                    <div
                      className={`w-12 h-12 rounded-2xl flex items-center justify-center overflow-hidden font-bold transition-all ${
                        isSelected
                    ...
                    {chat.type === 'direct' && chat.status && (
                      <div
                        className={`absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 rounded-full border-[2.5px] border-[#121215] ${STATUS_COLORS[chat.status] || STATUS_COLORS.offline}`}
                      />
                    )}
                  </div>
