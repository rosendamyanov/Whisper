...
export const CreateGroupModal = ({ onClose }: { onClose: () => void }) => {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      // Refresh sidebar chat list to show the new group immediately
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="w-full max-w-md bg-[#121215] border border-white/[0.08] rounded-2xl shadow-2xl flex flex-col max-h-[80vh]">
        ...
        <div className="p-4 space-y-4 flex-1 overflow-hidden flex flex-col">
          {/* Group Name Input */}
          <div>
            <label className="text-xs text-gray-500 uppercase font-bold mb-1.5 block ml-1">Group Name</label>
            <input 
              value={groupName}
          ...
          <div className="flex-1 overflow-y-auto min-h-0 custom-scrollbar">
            ...
            {isLoading ? (
                ...
                <div className="text-center text-gray-500 text-xs py-4">No friends found. Add some friends first!</div>
            ) : (
