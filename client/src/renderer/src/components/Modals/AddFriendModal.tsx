...
export const AddFriendModal = ({ onClose }: { onClose: () => void }): JSX.Element => {
        ...
        <div className="flex-1 overflow-y-auto p-4 pt-0 space-y-2 custom-scrollbar">
          {isLoading ? (
            ...
            searchResults.map((user) => (
              <div
                key={user.id}
                className="flex items-center justify-between p-3 rounded-xl bg-white/[0.03] hover:bg-white/[0.06] transition-colors border border-transparent hover:border-white/[0.05]"
              >
